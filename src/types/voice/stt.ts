 ...
 * Agents API each time — or `undefined`, when no token can be issued for the agent. It lets an
 * application drive its own speech recognition against the service the agent uses without putting
 * D-ID credentials in the browser. The token expires, so request one per recognition session
