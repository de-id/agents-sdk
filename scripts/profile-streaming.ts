...
/**
 * Streaming Manager Profiling Script (TypeScript)
 *
...
import { createAgentManager, enableStreamingProfiling } from '../src/index';
...
if (!agentId || !clientKey || !didApiUrl || !wsUrl) {
    ...
    console.error('\nExample:');
    console.error(
        '  VITE_AGENT_ID=your_agent_id VITE_CLIENT_KEY=your_key VITE_DID_API_URL=https://api.d-id.com VITE_WS_ENDPOINT=wss://api.d-id.com npx tsx scripts/profile-streaming.ts'
    );
    process.exit(1);
}
